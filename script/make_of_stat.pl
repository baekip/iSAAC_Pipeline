...
print $fh_stat "Delivery_ID\tTBI_ID\tIndex\tTotalReads\tTotalBases\tTotalBases(Gb)\tGC_Count\tGC_Rate\tN_ZeroReads\tN_ZeroReadsRate\tN5_LessReads\tN5_LessReadsRate\tN_Count\tN_Rate\tQ30_MoreBases\tQ30_MoreBasesRate\tQ20_MoreBases\tQ20_MoreBasesRate\n";

foreach my $id (@delivery_list) {
    my ($delivery_id,$tbi_id,$type_id) = split /\:/, $id;
    my $sample_path="$fastq_path/$tbi_id/";
    checkDir($sample_path);
    my $rst_xls="$sample_path/$tbi_id\_R1.fastq.gz.rst.xls";
    checkFile($rst_xls);
    ...
    my $sample_rawdata="$fastq_path/$tbi_id/$tbi_id\_R1.fastq.gz";
    ...
    print $fh_stat "$delivery_id\t$tbi_id\t$index\t$total_reads\t$total_length\t$total_base_gb\t$total_GC\t$gc_rate\t$Nzero_reads\t$Nzero_rate\t$N5_reads\t$N5_rate\t$N_count\t$N_rate\t$Q30_base\t$Q30_rate\t$Q20_base\t$Q20_rate\n";
}
close ($fh_stat);
print "End\n"; 
#############################################################
